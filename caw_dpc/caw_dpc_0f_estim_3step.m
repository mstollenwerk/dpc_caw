...
% Optimization
eparam.components.all(N+1:end) = ...
    fmincon(...
        @(param) sum_components_qlikeRec( ...
            param, gammas, p_c, q_c, diagLRL, enf_pd ...
        ), ...
        x0, ...
        A,b,[],[],lb,[],[], ...
...
[~,~,d,fcst_d] = sum_components_qlikeRec(...
    eparam.components.all(N+1:end),...
    gammas, p_c, q_c, diagLRL, enf_pd, fcst_steps...
 );


% Store estimated parameters in struct "eparam"
eparam.components.arch_param = eparam.components.all(:,N+1:1+p_c);
eparam.components.garch_param = eparam.components.all(:,N+1+p_c:1+p_c+q_c);
% Stored component intercepts are = (1-alphas-betas)*gammas. NOT gammas
% themselves.
eparam.components.all(1:N) = ...
    (1-sum(eparam.components.all(N+1:end))).*gammas; 
eparam.components.intrcpt = eparam.components.all(1:N);
...
% "logL".
aic = 2*nLogL + 2*(K + p_l + q_l + p_c + q_c + 1);
bic = 2*nLogL + log(K*T)*(K + p_l + q_l + p_c + q_c + 1);
logL = struct(...
    'logL', -nLogL,...
    'aic', aic,...
    'bic', bic,...
...
function [nLogL, logLcontr, d, fcst_d] = ...
    sum_components_qlikeRec(param, intrcpt, p_c, q_c, diagLRL, enf_pd, varargin)
...
for ii = 1:N
    [nLogL(ii), logLcontr(:,ii), d(:,ii), fcst_d(:,ii)] = ...
        components_qlikeRec( ...
            param, intrcpt(ii), p_c, q_c, diagLRL(:,ii), enf_pd, fcst_steps ...
        );
end
nLogL = sum(nLogL);
